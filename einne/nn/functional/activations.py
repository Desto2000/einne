...
class ProbabilisticActivation(nn.Module):
    ...
    def ripple(self, data, temperature=1.0):
        ...
        gradients = fe.clip_gradients(torch.autograd.grad(outputs=transformed.sum(), inputs=data, create_graph=True)[0])
    ...
    def ripplemoe(self, data, temperature=1.0):
        ...
        gradients = fe.clip_gradients(torch.autograd.grad(outputs=mix.sum(), inputs=data, create_graph=True)[0])
